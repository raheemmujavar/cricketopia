...
var redis_client = _redis.createClient(6379,redisip);
//settings
var settings = {
...
function isSquadAnnounced(matchId,callback){
  db.matchplayers.findOne({matchId:matchId},function(err,docs){
    if(err){
      console.log("error in getting squad " + err)
    }else{
      if(docs){
        callback("Yes");
      }else{
...
function getMyMatches(matchesList,callback){
  var mainArray = [];
  var tempDate,match= [],match1=[];
  db.match_shedules.find({matchId:{$in:matchesList}}).sort({StartDate : 1}).limit(6).exec(function(err,docs){
    if(err)console.log(err);
    else{
      if(docs){
        console.log(" get match new ---------------------------------------------------------")
...
client.on('setBidInformation',function(data){
 ...
 client.on('sellplayer',function(data){
  ...
  client.on("getLeagueLeaderBoard",function(data){
    console.log(data.leagueId)
    console.log(data.matchId)
    console.log(data.leagueId)
    var nagLeague = new Leaderboard('matchLeaderboard' + matchId + "_" + data.leagueId, {pageSize:10}, redis_client)
    nagLeague.list(function(err,result){
        if(err)
          console.log("error " + err)
        else{
          console.log(result)
          if(result){
            client.emit('getLeagueLeaderBoard',result);
          }else{
            client.emit('getLeagueLeaderBoard',[]);
          }

        }
      })
  })
  var globalLeaderBoard = new Leaderboard('fPLeaderboard', {pageSize:5}, redis_client)
  client.on("getLeaderBoard",function(data){
    // console.log("in le")
    var page = data.page | 0;
    var userId = data.userId;
    fantasyPointsLeaderBoard(globalLeaderBoard,function(){
      globalLeaderBoard.list(page,function(err,result){
        if(err)
          console.log("error " + err)
        else{
          ...
          if(result){
            ...
            // console.log(result)
            getLeaderBoardMembers(result,globalLeaderBoard,function(userLeaderBoard){
              console.log(userLeaderBoard)
              leaderBoardDetails.leaderboard = userLeaderBoard;
              leaderBoardUserPosition(globalLeaderBoard,userId,function(userInfo){
                leaderBoardDetails.userPosition = userInfo;
                client.emit('getLeaderBoard',leaderBoardDetails);
              })
            })
          }else{
            client.emit('getLeaderBoard',[]);
          }
  ...
  var friendsLeaderBoard = new Leaderboard('ffPLeaderboard', {pageSize:5}, redis_client)
  client.on("getFriendsLeaderBoard",function(data){
    var page = data.page | 0;
    var userId = data.userId;
    console.log(userId)
    frinedsFantasyPointsLeaderBoard(friendsLeaderBoard,userId,function(){
      friendsLeaderBoard.list(page,function(err,result){
        if(err)
          console.log("error " + err)
        else{
          var leaderBoardDetails = {};
          if(result){
            console.log("the result is ")
            // console.log(result)
            getLeaderBoardMembers(result,friendsLeaderBoard,function(userLeaderBoard){
              console.log(userLeaderBoard)
              leaderBoardDetails.leaderboard = userLeaderBoard;
              leaderBoardUserPosition(friendsLeaderBoard,userId,function(userInfo){
                leaderBoardDetails.userPosition = userInfo;
                client.emit('getFriendsLeaderBoard',leaderBoardDetails);
              })
            })
          }else{
            client.emit('getFriendsLeaderBoard',leaderBoardDetails);
  ...
  var locationLeaderBoard = new Leaderboard('lfPLeaderboard', {pageSize:5}, redis_client)
  client.on("getLocationLeaderBoard",function(data){
    var page = data.page | 0;
    var userId = data.userId;
    console.log(userId)
    locationFantasyPointsLeaderBoard(locationLeaderBoard,userId,function(){
      locationLeaderBoard.list(page,function(err,result){
        if(err)
          console.log("error " + err)
        else{
          var leaderBoardDetails = {};
          if(result){
            console.log("the result is ")
            // console.log(result)
            getLeaderBoardMembers(result,locationLeaderBoard,function(userLeaderBoard){
              console.log(userLeaderBoard)
              leaderBoardDetails.leaderboard = userLeaderBoard;
              leaderBoardUserPosition(locationLeaderBoard,userId,function(userInfo){
                leaderBoardDetails.userPosition = userInfo;
                client.emit('getLocationLeaderBoard',leaderBoardDetails);
              })
            })
          }else{
            client.emit('getLocationLeaderBoard',leaderBoardDetails);
  ...
  client.on("followMatch",function(data){
    var matchId = data.matchId;
    var userId = data.userId;
    console.log(userId)
    followMatch(matchId,userId,function(){
      client.emit('followMatch', "success");
    })
  })
  client.on("unFollowMatch",function(data){
    var matchId = data.matchId;
    var userId = data.userId;
    console.log(userId)
    unFollowMatch(matchId,userId,function(){
      client.emit('unFollowMatch', "success");
    })
  })
  client.on("getMyMatches",function(data){
    var userId = data.userId;
    getMyMatchesList(userId,function(matchesList){
      console.log(matchesList)
      getMyMatches(matchesList,function(matches){
        console.log(matches)
        client.emit('getMyMatches', "success");
      })
    })
...
function fantasyPointsLeaderBoard(globalLeaderBoard,callback){
  db.userSchema.find({},{_id:1,Credits:1},{sort:"FBID"},function(err,users){
    if(err){
      ...
      if(users){
        ...
        // console.log(users)
        function usersLoop(j){
          if(users[j].Credits === undefined)
            users[j].Credits = 0;
          globalLeaderBoard.add(users[j]._id,users[j].Credits,function(err){
            if(err)
              console.log("error in adding to leaderboard " + err)
            console.log(users[j]._id + " == " + users[j].Credits)
            console.log(j + " === " + nj)
            j++;
            if(j != nj)
              usersLoop(j);
...
function frinedsFantasyPointsLeaderBoard(friendsLeaderBoard,userId,callback){
  db.userSchema.findOne({_id:userId},{FriendsList:1,_id:0},{sort:"FBID"},function(err,userDoc){
    if(err){
      console.log("error in getting users")
    }else{
      if(userDoc){
        console.log(userDoc)
        var users = userDoc.FriendsList;
        var j=0;nj=users.length;
        // console.log(users)
        function usersLoop(j){
          db.userSchema.findOne({_id:users[j]},{_id:1,Credits:1},function(err,frinedDoc){
            if(err){
              console.log("error in getting users")
            }else{
              if(frinedDoc){
                if(frinedDoc.Credits === undefined)
                  frinedDoc.Credits = 0;
                friendsLeaderBoard.add(frinedDoc._id,frinedDoc.Credits,function(err){
                  if(err)
                    console.log("error in adding to leaderboard " + err)
                  console.log(frinedDoc._id + " == " + frinedDoc.Credits)
                  console.log(j + " === " + nj)
                  j++;
                  if(j != nj)
                    usersLoop(j);
...
function locationFantasyPointsLeaderBoard(locationLeaderBoard,userId,callback){
  db.userSchema.findOne({_id:userId},{FBCITY:1,_id:0},function(err,userDoc){
    if(err){
      console.log("error in getting users")
    }else{
      if(userDoc){
        console.log(userDoc)
        var location = userDoc.FBCITY;
        db.userSchema.find({FBCITY:location},{_id:1,Credits:1},function(err,users){
          if(err){
            console.log("error in getting users")
          }else{
            if(users){
              ...
              function usersLoop(j){
                if(users[j].Credits === undefined)
                  users[j].Credits = 0;
                locationLeaderBoard.add(users[j]._id,users[j].Credits,function(err){
                  if(err)
                    console.log("error in adding to leaderboard " + err)
                  console.log(users[j]._id + " == " + users[j].Credits)
                  console.log(j + " === " + nj)
              ...
              console.log("no users found in user location")
              callback();
            }
          }
        })
...
function getLeaderBoardMembers(members,leaderboardObj,callback){
  var i=0,ni=members.length;
  var userLeaderBoard = [];
  function memberLoop(i){
    var userData = {};
    var userId = members[i].member;
    db.userSchema.findOne({_id:userId},{FBNAME:1,FBPIC:1},function(err,userDoc){
      if(err){
        console.log("error in getting users profile " + err)
      }else{
        if(userDoc){
          ...
          // userData.userPic = userDoc.FBPIC; 
            ...
            userData.rank = rank; 
            userData.userScore = members[i].score; 
            userLeaderBoard.push(userData);
            i++;
            if(i != ni)
              memberLoop(i)
            if(i == ni){
...
function leaderBoardUserPosition(leaderboardObj,userId,callback){
  leaderboardObj.rank(userId,function(err,rank){
    console.log("rank of " + userId +" is " + rank);
    var rank = parseInt(rank)+1;
    var userData = {}
    db.userSchema.findOne({_id:userId},{FBNAME:1,FBPIC:1,Credits:1},function(err,userDoc){
      if(err){
        console.log("error in getting users profile " + err)
      }else{
        if(userDoc){
          ...
          userData.userName = userDoc.FBNAME; 
          // userData.userPic = userDoc.FBPIC; 
          userData.userScore = userDoc.Credits; 
          userData.rank = rank; 
          callback(userData);
        }
      }
...
function followMatch(matchId,userId,callback){
  db.MatchFollowers.findOne({matchId:matchId},function(err,matchInfo){
    if(err){
      console.log("error in getting follow match " + err)
    }else{
      if(matchInfo){
        db.MatchFollowers.findOne({matchId:matchId,users:{$elemMatch:{userId:userId}}},function(err,matchInfos){
            if(err){
              console.log("error in getting follow match " + err)
            }else{
              if(matchInfos){
                db.MatchFollowers.update({matchId:matchId,users:{$elemMatch:{userId:userId}}},{$set:{"users.$.isFollowed":"Yes"}},function(err1,matchUpdated){
                ...
                db.MatchFollowers.update({matchId:matchId},{$addToSet:{users:userInfo}},function(err1,matchUpdated){
                  console.log(matchUpdated);
                  callback();
                })
              }
        ...
        var matchFollower = {};
        matchFollower.matchId = matchId;
        matchFollower.users = [];
        var userInfo = {};
        userInfo.userId = userId;
        userInfo.isFollowed = "Yes";
        matchFollower.users.push(userInfo);
        var matchFollwers = new db.MatchFollowers(matchFollower)
        matchFollwers.save(function(err,rec){
          if(err)
            console.log("error in saving league " + err)
          else{
            console.log(rec)
...
function unFollowMatch(matchId,userId,callback){
  db.MatchFollowers.update({matchId:matchId,users:{$elemMatch:{userId:userId}}},{$set:{"users.$.isFollowed":"No"}},function(err1,matchUpdated){
    console.log(matchUpdated);
    callback();
  })
}
function getMyMatchesList(userId,callback){
  db.MatchFollowers.find({users:{$elemMatch:{userId:userId,isFollowed:"Yes"}}},{matchId:1,_id:0},function(err1,matchesList){
    if(err1){
      ...
      if(matchesList){
        ...
        for(var i=0;i<matchesList.length;i++){
          matches.push(matchesList[i].matchId)
        }
        callback(matches)
      }
      else{
